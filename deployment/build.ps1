# Lambda Deployment Package Builder — lambda-generate-pdf-invoice
# Run this script to create a deployment zip for AWS Lambda

Write-Host "Building Lambda deployment package (lambda-generate-pdf-invoice)..." -ForegroundColor Cyan
...
$deploymentPath = Join-Path $PSScriptRoot "lambda-generate-pdf-invoice-deployment.zip"

Compress-Archive -Path *.mjs,*.json,assets,config,generators,services,node_modules -DestinationPath $deploymentPath -Force
...
if (Test-Path $deploymentPath) {
    ...
    Write-Host "  aws lambda update-function-code --function-name shopify-generate-pdf-invoice --zip-file fileb://$deploymentPath" -ForegroundColor Gray
} else {
